...
import { warningSetting, saveWarningSetting } from '@/services/remote';
import IssuedButtons from "../../../components/IssuedButtons";
import { warningSettingConfig } from "../remoteTables";
...
const WarningSetting = props => {
  ...
  const getData = async (info) => {
    setLoading(true);
    const res = await warningSetting(info);
    if (res && res?.id) {
      setData(res);
      setDate(res.created)
    }
  ...
  const handleFinish = async (values) => {
    ...
    setLoading(true);
    const res = await saveWarningSetting(params);
    if (res && res?.id) {
    ...
    <Layout className="right_layout_content">
      ...
      >
        ...
        <Card style={{ margin: "16px 0px", padding: 24 }}>
          ...
          >
              {
                  Object.entries(warningSettingConfig).map(([key, val]) => {
                      return (
